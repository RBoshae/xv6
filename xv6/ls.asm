...
putc(int fd, char c)
{
 ...
 6c8:	e8 f5 fe ff ff       	call   5c2 <write>
    buf[i++] = digits[x % base];
  }while((x /= base) != 0);
  if(neg)
    buf[i++] = '-';
...
free(void *ap)
{
 ...
 891:	a1 94 0d 00 00       	mov    0xd94,%eax
static Header base;
static Header *freep;
...
free(void *ap)
{
 ...
 8e7:	89 08                	mov    %ecx,(%eax)
  freep = p;
 8e9:	a3 94 0d 00 00       	mov    %eax,0xd94
}
 8ee:	5b                   	pop    %ebx
 8ef:	5e                   	pop    %esi
 8f0:	5f                   	pop    %edi
 ...
 90a:	03 53 fc             	add    -0x4(%ebx),%edx
    ...
    p->s.ptr = bp;
  freep = p;
 90d:	a3 94 0d 00 00       	mov    %eax,0xd94
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
 ...
 929:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 92c:	8b 15 94 0d 00 00    	mov    0xd94,%edx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
